...
def _py_slow_minsn_to_ast(instruction: ida_hexrays.minsn_t) -> typing.Any | None:
    try:
        ...
        if instruction.opcode not in MBA_RELATED_OPCODES and not is_rotate_helper_call(
            ...
            # Log at info level for early_rule debugging
            if "D810.early_rule" in str(logger.name) or logger.debug_on:
                logger.info(
                    "[minsn_to_ast] Opcode %s @ 0x%x (%s) NOT in MBA_RELATED_OPCODES, returning None",
                    opcode_to_string(instruction.opcode),
                    instruction.ea,
                    instruction.dstr(),
                )
        ...
        # Log what create_from_insn produced
        if logger.debug_on:
            logger.debug(
                "[minsn_to_ast] create_from_insn: instruction opcode=%s, ins_mop.t=%s, ins_mop.dstr=%s",
                opcode_to_string(instruction.opcode),
                mop_type_to_string(ins_mop.t),
                str(ins_mop.dstr()) if hasattr(ins_mop, "dstr") else "N/A",
            )
